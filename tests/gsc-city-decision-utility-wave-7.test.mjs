...
const ROUTES = {
  london: ['london.ca', 'www.canada.ca'],
  'st-john-s': ['www.stjohns.ca'],
  'el-paso': ['www.elpasotexas.gov'],
  fredericton: ['www.fredericton.ca'],
  norfolk: ['www.norfolk.gov'],
  'long-beach': ['www.longbeach.gov'],
  'thunder-bay': ['subscription.thunderbay.ca', 'www.canada.ca'],
};
...
test('seventh GSC city wave adds distinct official-source utility without publishing providers', async () => {
  ...
  for (const [slug, sourceHosts] of Object.entries(ROUTES)) {
    ...
    assert.equal(externalLinks.length, 2, `${route}: expected exactly two official sources`);
